...
def main():
	...
	### scan 2 ###
	x2 = np.zeros((len(scan2),1))
	y2 = np.zeros((len(scan2),1))
	z2 = np.zeros((len(scan2),1))
	r2 = np.zeros((len(scan2),1))
	one2 = np.zeros((len(scan2),1))
	zero2 = np.zeros((len(scan2),1))
	...
	### scan 3 ###
	x3 = np.zeros((len(scan3),1))
	y3 = np.zeros((len(scan3),1))
	z3 = np.zeros((len(scan3),1))
	r3 = np.zeros((len(scan3),1))
	one3 = np.zeros((len(scan3),1))
	zero3 = np.zeros((len(scan3),1))

	for i, row in enumerate(scan3):
		_x = row[0]
		_y = row[1]
		_z = row[2]
		_r = row[3] / 255.0
	...
	### scan 4 ###
	x4 = np.zeros((len(scan4),1))
	y4 = np.zeros((len(scan4),1))
	z4 = np.zeros((len(scan4),1))
	r4 = np.zeros((len(scan4),1))
	one4 = np.zeros((len(scan4),1))
	zero4 = np.zeros((len(scan4),1))

	for i, row in enumerate(scan4):
		_x = row[0]
		_y = row[1]
		_z = row[2]
		_r = row[3] / 255.0
	...
	for i, row in enumerate(pre_rotation_data):
		rows[i,:] = i
		_roll = args.roll * np.pi/180 # degrees to radians
		_pitch = args.pitch * np.pi/180 # degrees to radians
		_yaw = args.yaw * np.pi/180 # degrees to radians
		_x1 = row[0]
		_y1 = row[1]
		_z1 = row[2]
	...
	reflectivity_2 = np.concatenate((zero2, one2, zero2, r2), axis=1)

	x_y_z_3 = np.concatenate((x3, y3, z3), axis=1)
	reflectivity_3 = np.concatenate((zero3, zero3, one3, r3), axis=1)

	x_y_z_4 = np.concatenate((x4, y4, z4), axis=1)
	reflectivity_4 = np.concatenate((one4, one4, zero4, r4), axis=1)
